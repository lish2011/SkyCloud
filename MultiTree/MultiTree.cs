...
namespace MultiTree
{
    ...
    public class MultiTree
    {
        ...
        //public List<int> SeqIndex = new List<int>();
        // 深度优先遍历
        private static List<string> Traverse(MTreeNode head)
        {
            List<string> SeqIndex = new List<string>();
            if (head == null)
            {
                return SeqIndex;
            }
            SeqIndex.Add(head.Name);
            foreach (var child in head.Children)
            {
                SeqIndex.AddRange(Traverse(child));
            }
            return SeqIndex;
        }
...
// https://blog.csdn.net/guigenyi/java/article/details/46761273
}
